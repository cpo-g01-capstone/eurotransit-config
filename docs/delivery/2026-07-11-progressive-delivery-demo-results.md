...
*Execution record for T6 (ADR 0026, runbook in `progressive-delivery-runbook.md`) plus
the first k6 baseline (T9). Every phase below is a merged PR — the rollout history IS
...
## 1. k6 baseline (T9) — first official run
...
(slow-call 2 s) are conservative, and gives the D6 canary gate (300 ms) a 3×
margin — both tunable after the chaos runs.

## 2. Canary on Orders — D6 gate held → PROMOTE
...
## 3. Blue/green on Catalog — clean D7 window → cleanup
...
**D7 window** (opened 17:26:21Z, 5 clean minutes under k6):
...
`order-failed(SOLD_OUT)` → Orders marks the order FAILED — D4 + the BUG-3 fix) ran
...
## 4. What this buys the chaos sessions (T1)
